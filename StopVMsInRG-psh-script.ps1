...
Write-Output ('Start Shutdown of VMs in RessourceGroup {0} - {1:HH}:{1:mm}' -f $ResourceGroupName, (Get-Date))
...
if (!$VMs) {
    ...
    Foreach ($VM in $VMs) {
        ...
        if (($VM | Stop-AzureRmVM -Force -ErrorAction SilentlyContinue).IsSuccessStatusCode) {
            Write-Output ('Stopped {0}' -f $VM.Name)
        } else {
            Write-Output ('Failed to stop {0}' -f $VM.Name)
        }
    }
}
Write-Output ('DONE - Shutdown of VMs in RessourceGroup {0} - {1:HH}:{1:mm}' -f $ResourceGroupName, (Get-Date))
